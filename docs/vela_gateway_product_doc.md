...
> Vela 是一个轻量、可嵌入、基于 Provider 驱动的动态应用网关。

Vela 的目标不是简单复制 Traefik 或 Caddy，而是面向私有化部署、自研编排系统、裸机服务治理、Docker/Swarm 场景，提供一个同时支持 **独立二进制运行** 与 **嵌入式集成** 的轻量应用网关。

### 1.2 一句话定位

**Vela 是一个可嵌入、可独立部署、支持私有插件生态的轻量动态应用网关。**
...
因此，Vela 的目标是提供一个更贴合自研平台需求的轻量应用网关内核。
...
Vela 第一阶段不追求成为完整 API Gateway 或 Service Mesh。
...
Vela 的第一原则是：**先成为稳定、轻量、可控的入口流量运行时。**
...
`orch` 负责服务调度、实例生命周期和服务注册，Vela 作为嵌入式 Gateway Runtime 负责接收外部请求并转发到对应服务实例。
...
Vela 以 `velad` 的形式运行，通过 HCL/YAML 文件或 Docker/Swarm Provider 加载服务路由配置。
...
企业内部自建插件 Registry，开发认证、签名、限流、审计等插件，并由 Vela 动态加载。
...
多个 Vela 节点组成 Gateway Cluster，Manager 节点负责配置和状态同步，Proxy 节点负责流量转发。
...
Vela 不把配置文件作为唯一配置来源，而是将所有配置来源抽象为 Provider。
...
labels:
  vela.enable: "true"
  vela.http.routers.api.rule: "Host(`api.example.com`)"
  vela.http.routers.api.service: "api"
  vela.http.services.api.loadbalancer.server.port: "8080"
```
...
Vela 插件系统的目标是提供一个可私有化、可版本化、可校验、可隔离的扩展机制。
...
```text
                 ...
                 +----------+-----------+
                            ...
                            v
+--------+    +----------------------+    +-----------+
| Client | -> |  Vela Proxy Runtime  | -> | Upstream  |
+--------+    +----------------------+    +-----------+
```
...
```hcl
plugin "corp-authz" {
  source  = "oci://registry.example.com/vela/plugins/authz"
  version = "1.0.0"
}
...
```bash
vela version
vela validate -f vela.hcl
vela run -f vela.hcl
velad --config vela.hcl
```

### 10.2 插件命令

```bash
vela plugin search authz
vela plugin install oci://registry.example.com/vela/plugins/authz:1.0.0
vela plugin list
vela plugin inspect corp/authz
vela plugin remove corp/authz
vela plugin upgrade corp/authz --version 1.1.0
```
...
```bash
vela cluster init
vela cluster join --manager http://10.0.0.1:9900
vela cluster members
vela cluster status
```
...
vela_upstream_request_duration_seconds
...
Vela 的核心价值不是替代所有网关，而是解决现有网关在私有化、自研编排系统、嵌入式运行、插件分发和控制面集群化上的不匹配问题。
...
Vela 不应该从一开始就追求大而全，而应该先把动态路由、服务发现、热更新、健康检查、访问日志和嵌入式运行做好。
